...
> **Ограничение:** В текущей версии одна сессия привязана к одному студенту. Поддержка нескольких студентов в одной сессии — планируется.
...
| Метод | Endpoint                              | Описание            | Роли |
|--------|---------------------------------------|------------------------|-------|
| GET    | /violations/session/{sessionId}       | Список нарушений для сессии (студент берётся из сессии) | All   |
| POST   | /violations                           | Reported нарушение (привязано к sessionId)       | PROCTOR |
| POST   | /violations/{id}/review               | Просмотр нарушения       | PROCTOR, ADMIN |
| GET    | /violations/stats/{sessionId}         | Статистика по сессии             | All   |
